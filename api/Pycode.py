...
def get_timeline_data():
    ...
    for event in raw_timeline:
        ...
        for sector_name, base_price in baselines.items():
            # 為了讓前端 Chart.js 好讀取，我們只把 14 天的 price 數字抽成純陣列
            raw_trends = generate_sector_fluctuation(base_price, sector_name)

            # 依據前端需要的簡短 key 值做精準對應
            if sector_name == "Ballistic Materials (UHMWPE/Ceramic)":
                sector_graphs["materials"] = [d["price"] for d in raw_trends]
            elif sector_name == "Defense Weapons":
                sector_graphs["weapons"] = [d["price"] for d in raw_trends]
            elif sector_name == "Drones":
                sector_graphs["drones"] = [d["price"] for d in raw_trends]
            elif sector_name == "Semiconductors":
                sector_graphs["semis"] = [d["price"] for d in raw_trends]

        # 💡 重大修正：將 market_analytics 改為 market_impact，並補上 tracked_assets
        final_timeline.append({
            ...
            "market_impact": {
                "analysis": "Defense (Drones/Weapons) and protective material suppliers (UHMWPE/Ceramics) show sharp 48-hour demand spikes, contrasting with broader electronic and semiconductor dips.",
                "tracked_assets": [
                    {
                        "ticker": "TPE:2330",
                        "trend": "Resilient (AI/Hardware Demand Supported)"
                    }
                ],
                "chart_data": sector_graphs  # 完美對應前端的 event.market_impact.chart_data
            }
        })
